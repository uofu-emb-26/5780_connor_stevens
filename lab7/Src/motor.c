...
void PI_update(void) {
     ...
     */
     ...
     // motor-speed is in pules per interrupt which happens 1600 times per minute, so motor_speed * 1600 is motor_speed per minute
     // Converting rpm to pulses per minute requires multiplying by 3200 (3200 pulses per rotation)
